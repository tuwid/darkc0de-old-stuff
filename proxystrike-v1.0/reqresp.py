...
class Request:
	...
	def __getattr__ (self,name):
		if name=="urlWithoutVariables":
			return urlunparse((self.__schema,self.__host,self.__path,'','',''))
		elif name=="pathWithVariables":
			return urlunparse(('','',self.__path,'',self.getVARIABLESstring(self.__variablesGET),''))
		elif name=="completeUrl":
			return urlunparse((self.__schema,self.__host,self.__path,'',self.getVARIABLESstring(self.__variablesGET),''))
		elif name=="urlWithoutPath":
			return "%s://%s" % (self.__schema,self.__headers["Host"])
		elif name=="path":
			return self.__path
		elif name=="postdata":
			if self.ContentType=="application/x-www-form-urlencoded":
				return self.getVARIABLESstring(self.__variablesPOST)
	...
	def getVARIABLESstring(self,vars):
		str=[]
	...
	def setUrl (self, urltmp):
		...
		self.__schema,self.__host,self.__path,d,variables,f=urlparse(urltmp)

		self.__headers["Host"]=self.__host

		if len(variables)>0:
			self.__variablesGET.update(self.readUrlEncodedVariables(variables))
...
############################################################################
	...
	def perform(self):
		...
		if self.followLocation:
			if self.response.getLocation():
				...
				if "Cookie" in self.__headers:
					a.addHeader("Cookie",self.__headers["Cookie"])
				a.perform()
	...
	def Substitute(self,src,dst):
		...
		b=rx.sub(dst,a)
		del rx
		self.parseRequest(b,self.__schema)
