...
for i in encs:
	try:
		if i[:8]=='encoder_':
			ENCODERS[getattr(encoders,i).text]=i
	except:
		pass
...
class FuzzResult:
	...
	def __init__(self,request,saveMemory=True):
		...
		if __name__=="__main__":
			...
			imprimeResult (nreq,self.code,self.lines,self.words,request.description[-50:],fl)

			if html:
				if str(self.code) in hidecodes or str(self.lines) in hidelines or str(self.words) in hidewords:
					return
				imprimeResultHtml (nreq,self.code,self.lines,self.words,request)
...
-e encoding : Encoding for dictionary (-e help for a list of encodings)
...
Keyword: FUZZ,FUZ2Z  wherever you put these words wfuzz will replace them by the dictionary.
...
''' % (sys.argv[0])
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "cx:b:e:R:d:z:r:f:t:w:V:H:",['hc=','hl=','hw=','ntlm=','basic=','digest=','html'])
		optsd=dict(opts)
		if "-e" in optsd:
			if optsd["-e"] == "help":
	...
	if "-H" in optsd:
		...
		for x in headers:
			splitted=x.split(":")
			a.addHeader(splitted[0],splitted[1])
